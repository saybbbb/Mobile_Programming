...
export default function ClassDetails() {
  ...
  const [sheet, setSheet] = useState<"student" | "bulletin" | "task" | null>(
    null
  );
  ...
  const openSheet = (type: typeof sheet) => {
    setSheet(type);
    const y =
      type === "student" ? studentY : type === "bulletin" ? bulletinY : taskY;
    Animated.timing(y, {
      toValue: 0,
      duration: 250,
  ...
  const closeSheet = () => {
    const y =
      sheet === "student" ? studentY : sheet === "bulletin" ? bulletinY : taskY;
    Animated.timing(y, {
  ...
  const saveStudent = async () => {
    ...
    if (editingStudent) {
      await api.put(`/classes/students/${editingStudent._id}`, {
        name: studentName,
        studentId,
      });
    } else {
  ...
  const saveTask = async () => {
    ...
    if (editingTask) {
      await api.put(`/classes/tasks/${editingTask._id}`, {
        title: taskTitle,
        dueDate: taskDue,
      });
    } else {
      await api.post(`/classes/${classid}/tasks`, {
        title: taskTitle,
        dueDate: taskDue,
      });
    }
  ...
  return (
    <View style={styles.container}>
      ...
      {menuStudent && (
        <TouchableWithoutFeedback onPress={() => setMenuStudent(null)}>
          <View style={styles.overlay}>
            <View style={styles.menu}>
              <TouchableOpacity
                style={styles.menuItem}
                onPress={() => {
                  setEditingStudent(menuStudent);
                  setStudentName(menuStudent.name);
                  setStudentId(menuStudent.studentId ?? "");
                  setMenuStudent(null);
                  openSheet("student");
                }}
              >
      ...
      {/* STUDENT SHEET */}
      <Animated.View
        style={[styles.sheet, { transform: [{ translateY: studentY }] }]}
      >
      ...
      {/* BULLETIN SHEET */}
      <Animated.View
        style={[styles.sheet, { transform: [{ translateY: bulletinY }] }]}
      >
      ...
      {/* TASK SHEET */}
      <Animated.View
        style={[styles.sheet, { transform: [{ translateY: taskY }] }]}
      >
        <Text style={styles.sheetTitle}>
